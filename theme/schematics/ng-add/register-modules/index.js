...
function registerModules(options) {
    ...
    return schematics_1.chain([registerAnimationsModule(options), registerNebularModules(options), registerRouterIfNeeded(options)]);
}
    ...
    return function (tree, context) {
        ...
        if (options.animations) {
            ...
            if (schematics_2.hasNgModuleImport(tree, appModulePath, noopAnimationsModuleName)) {
                return context.logger.warn(terminal_1.red("Could not set up \"" + terminal_1.bold(browserAnimationsModuleName) + "\" " + ("because \"" + terminal_1.bold(noopAnimationsModuleName) + "\" is already imported. Please manually ") + "set up browser animations."));
            }
