...
function insertThemeImportInStyles(tree, stylesPath) {
    var recorder = tree.beginUpdate(stylesPath).insertLeft(0, theme_content_1.stylesContent);
...
function noNebularThemeIncluded(targetOptions, stylesPath) {
    var existingStyles = targetOptions.styles.map(function (s) {
        return typeof s === 'string' ? s : s.input;
    });
    var hasGivenTheme = existingStyles.find(function (s) {
        return s.includes(stylesPath);
    });
    var hasOtherTheme = existingStyles.find(function (s) {
        return s.includes('@nebular/theme/styles/prebuilt');
    });
