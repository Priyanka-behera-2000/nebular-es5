...
function wrapInlineTemplate(tree, templateDescriptor) {
    var templateProp = templateDescriptor.templateProp,
        componentPath = templateDescriptor.componentPath,
        template = templateDescriptor.template;
    var wrappedTemplate = layout_content_1.wrapInlineTemplateInLayout(template);
    var recorder = tree.beginUpdate(componentPath).remove(templateProp.initializer.pos, template.length).insertLeft(templateProp.initializer.pos, wrappedTemplate);
    tree.commitUpdate(recorder);
}
function wrapHtmlFileTemplate(tree, templateDescriptor) {
    var templateUrlProp = templateDescriptor.templateUrlProp,
        componentPath = templateDescriptor.componentPath,
        template = templateDescriptor.template;
