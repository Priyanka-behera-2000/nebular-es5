"use strict";

Object.defineProperty(exports, "__esModule", {
    value: true
});
exports.NbSelectModule = undefined;

var _typeof = typeof Symbol === "function" && typeof Symbol.iterator === "symbol" ? function (obj) { return typeof obj; } : function (obj) { return obj && typeof Symbol === "function" && obj.constructor === Symbol && obj !== Symbol.prototype ? "symbol" : typeof obj; };

var _core = require("@angular/core");

var _cdk = require("../cdk");

var _shared = require("../shared/shared.module");

var _input = require("../input/input.module");

var _card = require("../card/card.module");

var _checkbox = require("../checkbox/checkbox.module");

var _button = require("../button/button.module");

var _select = require("./select.component");

var _option = require("./option.component");

var _optionGroup = require("./option-group.component");

var _icon = require("../icon/icon.module");

var __decorate = undefined && undefined.__decorate || function (decorators, target, key, desc) {
    var c = arguments.length,
        r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc,
        d;
    if ((typeof Reflect === "undefined" ? "undefined" : _typeof(Reflect)) === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);else for (var i = decorators.length - 1; i >= 0; i--) {
        if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    }return c > 3 && r && Object.defineProperty(target, key, r), r;
};

var NB_SELECT_COMPONENTS = [_select.NbSelectComponent, _option.NbOptionComponent, _optionGroup.NbOptionGroupComponent, _select.NbSelectLabelComponent];
var NbSelectModule = /** @class */function () {
    function NbSelectModule() {}
    NbSelectModule = __decorate([(0, _core.NgModule)({
        imports: [_shared.NbSharedModule, _cdk.NbOverlayModule, _button.NbButtonModule, _input.NbInputModule, _card.NbCardModule, _checkbox.NbCheckboxModule, _icon.NbIconModule],
        exports: NB_SELECT_COMPONENTS.slice(),
        declarations: NB_SELECT_COMPONENTS.slice()
    })], NbSelectModule);
    return NbSelectModule;
}();
exports.NbSelectModule = NbSelectModule;
//# sourceMappingURL=select.module.js.map
//# sourceMappingURL=select.module.js.map